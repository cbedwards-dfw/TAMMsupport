...
#' @export
#'
read_limiting_stock <- function(file, longform = FALSE) {
  column.names <- c(
    "Fishery", "FisheryID",
    file |>
  ...
  res <- dplyr::bind_rows(
    readxl::read_excel(file, range = "LimitingStkComplete mod!J5:DL76", col_names = column.names, .name_repair = "unique_quiet") |>
      dplyr::mutate(stock_type = "first_section"),
    readxl::read_excel(file, range = "LimitingStkComplete mod!J84:DL155", col_names = column.names, .name_repair = "unique_quiet") |>
      dplyr::mutate(stock_type = "ALL_H"),
    readxl::read_excel(file, range = "LimitingStkComplete mod!J163:DL234", col_names = column.names, .name_repair = "unique_quiet") |>
      dplyr::mutate(stock_type = "UM_H"),
    readxl::read_excel(file, range = "LimitingStkComplete mod!J242:DL313", col_names = column.names, .name_repair = "unique_quiet") |>
      dplyr::mutate(stock_type = "UM_N"),
    readxl::read_excel(file, range = "LimitingStkComplete mod!J321:DL392", col_names = column.names, .name_repair = "unique_quiet") |>
      dplyr::mutate(stock_type = "AD_H"),
    readxl::read_excel(file, range = "LimitingStkComplete mod!J400:DL471", col_names = column.names, .name_repair = "unique_quiet") |>
...
#'
#' @param file Name (and path) for TAMM files
#' @importFrom rlang .data
#' @return dataframe
#' @export
#'
clean_limiting_stock <- function(file) {
  dat <- read_limiting_stock(file)
  run.num <- readxl::read_excel(file, range = "1!B2", col_names = FALSE, .name_repair = "unique_quiet")
