...
format_overview <- function(dat_overview) {
  ...
  vec.indent <- c(
    "NF NOOKSACK SPRING", "SF NOOKSACK SPRING", "Upper Sauk", "Upper Cascade",
...
kable_overview <- function(dat_overview, ind_indent, col_bold) {
  kableExtra::kbl(dat_overview,
    ...
    kableExtra::add_indent(ind_indent, level_of_indent = 1) |> # this line needs to be before any extra_css
...
quick_er_format <- function(x) {
  ...
  suppressWarnings({
    can.numeric <- !is.na(as.numeric(x))
  })
