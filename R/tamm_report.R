...
#'
#' @param tamm.path Absolute path to directory containing the tamm file. (hoping to streamline this at some point). Character atomic.
#' @param tamm.name Name of tamm file (including .xlsx suffix). Character atomic
#' @param clean Should the .qmd files used to make the report be deleted afterwards? Logical, defaults to `TRUE`. Set to `FALSE`
...
#' tamm_report(tamm.path = tamm.path, tamm.name = "Chin2124.xlsx")
#' }
tamm_report <- function(tamm.path, tamm.name,  clean = TRUE){
  ...
  file.copy (c(qmd.path, qmd.child.path), tamm.path)
  ...
  quarto::quarto_render(paste0(tamm.path,"/tamm-visualizer.qmd"),
                        execute_params = list(tamm.path = paste0(tamm.path, "/", tamm.name)))
  file.rename(paste0(tamm.path,"/tamm-visualizer.html"),
              paste0(tamm.path, "/", report.name))

  if(clean){
    cli::cli_alert("Deleting .qmd files.")
