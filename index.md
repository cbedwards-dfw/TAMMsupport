...
- Summarize and visualize key aspects of a Chinook TAMM? Use
  ...
  (OR, even better, check out the [crayolaTAMM
...
By default,
[`tamm_diff()`](https://cbedwards-dfw.github.io/TAMMsupport/reference/tamm_diff.md)
flags changes that were at least 0.1% of the value in `file_1` (or, if
that value was zero, to within an absolute value of 0.001). In some
cases, we may want to solely focus on larger changes (e.g. changes in
whole fish). We can change the minimum threshold for proportional change
with with the argument `proportional_threshold`: a value of 0.1 would
mean only flagging changes of 10% or greater, and a value of 0.01 would
flag change of 1% or greater.
...
- [`read_2a_sheets()`](https://cbedwards-dfw.github.io/TAMMsupport/reference/read_2a_sheets.md)
  parses the sheets “2A_Cmrkd”, “2A_CUnmrkd”, “2A_CUandM” and
  “2A_CUandM_N”, extracts the AEQs and ERs, and labels them with the
  appropriate stocks and fisheries. This makes it much easier to writes
  scripts that depend on these values. It returns a list with `$AEQ`
  (all the AEQ values in tabular form) and `$ER` (all the ER values in
  tabular form)
- [`read_jdf()`](https://cbedwards-dfw.github.io/TAMMsupport/reference/read_jdf.md)
  is analogous to
  [`read_2a_sheets()`](https://cbedwards-dfw.github.io/TAMMsupport/reference/read_2a_sheets.md)
  but parses the AEQ information in the “JDF” sheet. For compatibility
  reasons it also returns a list with `$AEQ` and `$ER` like
  [`read_2a_sheets()`](https://cbedwards-dfw.github.io/TAMMsupport/reference/read_2a_sheets.md)
  does, but as there is not a bottom bar with ER values on the JDF
  sheet, the `$ER` entry is empty
...
`tamm-visualizer-child.qmd`, which will be leftin the same folder as the
TAMM and report.
