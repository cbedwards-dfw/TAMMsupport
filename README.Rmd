...
I want to...
 ...
 - compare three Chinook TAMM files (e.g. 3 ocean options) to see how ER varies by stock and fishery between the models? Use `tamm_compare3()`.
  - extract the overview sheet info from a Chinook TAMM? Use `read_overview()` or `read_overview_complete()`.
 - extract the limiting stock complete sheet from a Chinook TAMM? Use `read_limiting_stock()` or `clean_limiting_stock()`. Use `filter_tamm_wa()` to then filter the results to just Washington fisheries.
