...
## read in `tamm_names`  and `fisheries` by sourcing our external parameter files
source(paste0(tamm_path, "/tamm-compare-parameters-intermediate.R"))

tamms.full <- paste0(tamm_path, "/", tamm_names)
...
# Comparing `r paste0(tamm_names, collapse = ", ")`
...
for (i in 1:length(tamms.full)) {
  dat.ls[[i]] <- TAMMsupport::clean_limiting_stock(tamms.full[i]) |>
    dplyr::mutate(tamm = tamm_names[i])
  if (!is.null(fisheries)) {
    dat.ls[[i]] <- dat.ls[[i]] |>
...
render_child <- function(cur.tamm) {
  res <- knitr::knit_child(
    ...
    envir = rlang::env(cur.tamm = cur.tamm, tamm_path = tamm_path),
    quiet = TRUE
  )
  cat(res, sep = "\n")
  cat("\n")
...
#| results: 'asis'
#| echo: false
purrr::walk(tamm_names, render_child)
```
:::
